...
export function HistoryDatePage() {
  ...
  async function loadRange(from: string, to: string) {
    ...
    try {
      ...
      setError(tr("Не удалось загрузить историю по датам.", "Не вдалося завантажити історію за датами."));
    } finally {
      setIsLoading(false);
    }
  }
    ...
    <section className="screen settings-screen">
      ...
      <div className="list">
        ...
        {!isLoading && !error && items.length === 0 && (
          <div className="card__hint">{tr("За этот период игр не найдено.", "За цей період ігор не знайдено.")}</div>
        )}
